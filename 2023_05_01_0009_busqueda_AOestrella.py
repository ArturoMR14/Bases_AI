...
def ao_star(start, goal, neighbors_fn, heuristic_fn):
    frontier = [(0, start, math.inf)]
    came_from = {}
    cost_so_far = {start: 0}
    iteration = 0
    ...
    while frontier:
        ...
        _, current, fmin = heapq.heappop(frontier)
        ...
        for neighbor, cost in neighbors_fn(current):
            ...
            if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                ...
                f = new_cost + heuristic_fn(neighbor, goal)
                if f < fmin:
                    fmin = f
                heapq.heappush(frontier, (f, neighbor, fmin))
                came_from[neighbor] = current

    return None
# Establecer los nodos de inicio y fin del grafo
start_node = 'A'
end_node = 'L'

# Ejecutar la búsqueda AO* y mostrar la ruta encontrada
path = ao_star(start_node, end_node, get_neighbors, euclidean_distance)
